...
while(1):
    ...
    How it works: 
        ...
        3- group each group by time (1 minutes)
    ...
    for name in symbols:
        ...
        df=data.loc[data['code']==name,'price'].resample('1Min').ohlc()
        df['volume']=data.loc[data['code']==name,'volume'].resample('1Min').sum()
        #print(vwap)
        df['vwap'] = (vwap/data.loc[data['code']==name,'volume'].cumsum()).resample('1Min').last()
        #print(df)
        name=name.replace('.CA','').strip()
        #print('1',df)
        df=df.dropna()
        sql='SELECT * FROM STOCK.FILL_OHLCV_1MIN WHERE Ticker = :name ORDER BY BARTIMESTAMP DESC'
        ...
        if(len(tbUpd)>0 and res[5]!=tbUpd['volume'][0]):
            sql='update STOCK.FILL_OHLCV_1MIN set OPEN=:1,HIGH=:2,LOW=:3,CLOSE=:4,VOLUME=:5 where Ticker = :6 AND BARTIMESTAMP=:7'
        ...
        if(len(tbIns)>0):
            ...
            for index,row in tbIns.iterrows():
                try:
                    ...
                    #print(lines)
                    cursor2.execute("insert into FILL_OHLCV_1MIN(TICKER,OPEN,HIGH,LOW,CLOSE,VOLUME,BARTIMESTAMP,ASSET,VWAP) values (:stock, :open,:high,:low,:close,:vol,:time,:1,:2)",line)
                    con2.commit()
    ...
    try:
        for name in indicies:
            ...
            sql="SELECT BARTIMESTAMP,SUM(VOLUME) FROM STOCK.FILL_OHLCV_1MIN WHERE Ticker IN (%s)  GROUP BY BARTIMESTAMP ORDER BY BARTIMESTAMP DESC"  % ','.join(placeholders)
            #print(sql)
